document.addEventListener('DOMContentLoaded', () => {
    ...
    function resetForm() {
        ...
        formName.dataset.autoGenerated = 'true';
        showFormMessage('');
    }

    function normalizeComarcaLabel(value) {
        return String(value || '')
            .trim()
    ...
    function buildDiligenciaNameFromComarca(comarcaValue) {
        const base = normalizeComarcaLabel(comarcaValue);
        if (!base) {
            return '';
        }
        return `COMARCA DE ${base.toUpperCase()}`;
    }

    function syncCadastroNameFromComarca() {
        const generatedName = buildDiligenciaNameFromComarca(formComarca.value);
        const canAutoUpdate = !formName.value.trim() || formName.dataset.autoGenerated === 'true';
        if (!generatedName || !canAutoUpdate) {
            return;
        }
        formName.value = generatedName;
        formName.dataset.autoGenerated = 'true';
    }
    ...
    function updateKpis() {
        ...
        const totalArrecadado = dataset.reduce((sum, item) => sum + (Number(item.valor_alvara) || 0), 0);
    ...
    formComarca.addEventListener('input', syncCadastroNameFromComarca);
    formName.addEventListener('input', () => {
        if (!formName.value.trim()) {
            formName.dataset.autoGenerated = 'true';
            return;
        }
        const expectedAutoName = buildDiligenciaNameFromComarca(formComarca.value);
        formName.dataset.autoGenerated = String(formName.value.trim() === expectedAutoName);
    });
